/**
 * Merges curated Americas clubs into public/data/clubs/{countryCode}.json.
 * Run: npx tsx scripts/merge-americas-curated.ts
 */
...
import {
  AMERICAS_CURATED,
  getAmericasClubLocation,
  type AmericasClubEntry,
} from './data/americas-clubs.js'
...
function toClub(
  ...
  const curatedLocation = getAmericasClubLocation(countryCode, entry.name)
  ...
  return {
    ...
    lng: geo.lng,
    divisionTier: tier,
    leagueId: `americas-curated-${countryCode}-t${tier}`,
    leagueName,
    season: SEASON,
    logoUrl: teamId ? logoUrlForTeam(teamId) : '',
    source: 'americas-curated',
    updatedAt: TODAY,
  }
}
...
async function main() {
  ...
  await buildManifest()
  console.log(`\nMerged ${added} curated Americas clubs.`)
}
