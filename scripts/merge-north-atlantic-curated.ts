/**
 * Merges curated Greenland and Ireland clubs into public/data/clubs.
 * Run: npx tsx scripts/merge-north-atlantic-curated.ts
 */
import fs from 'fs/promises'
import path from 'path'
import { fileURLToPath } from 'url'
import {
  getNorthAtlanticClubLocation,
  NORTH_ATLANTIC_CURATED,
  type NorthAtlanticClubEntry,
} from './data/north-atlantic-clubs.js'
...
function toClub(
  ...
  const curatedLocation = getNorthAtlanticClubLocation(countryCode, entry.name)
  ...
  return {
    ...
    lng: geo.lng,
    divisionTier: tier,
    leagueId: `north-atlantic-curated-${countryCode}-t${tier}`,
    leagueName,
    season: SEASON,
    logoUrl: teamId ? logoUrlForTeam(teamId) : '',
    source: 'north-atlantic-curated',
    updatedAt: TODAY,
  }
}
...
async function main() {
  ...
  await buildManifest()
  console.log(`\nMerged ${added} curated North Atlantic clubs.`)
}
