/**
 * Merges curated Asian clubs into public/data/clubs/{countryCode}.json
 * Run: npx tsx scripts/merge-asia-curated.ts
 */
...
function toClub(
  ...
  return {
    ...
    lng: geo.lng,
    divisionTier: tier,
    leagueId: `asia-curated-${countryCode}-t${tier}`,
    leagueName,
    season: SEASON,
...
async function main() {
  ...
  for (const [countryCode, config] of Object.entries(ASIA_CURATED)) {
  ...
  await buildManifest()
  console.log(`\nMerged ${added} curated Asian clubs.`)
}
