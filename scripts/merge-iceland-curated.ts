/**
 * Merges curated Icelandic clubs into public/data/clubs/IS.json.
 * Run: npx tsx scripts/merge-iceland-curated.ts
 */
import fs from 'fs/promises'
import path from 'path'
import { fileURLToPath } from 'url'
import {
  getIcelandClubLocation,
  ICELAND_CURATED,
  type IcelandClubEntry,
} from './data/iceland-clubs.js'
...
function toClub(
  ...
  const curatedLocation = getIcelandClubLocation(countryCode, entry.name)
  ...
  return {
    ...
    lng: geo.lng,
    divisionTier: tier,
    leagueId: `iceland-curated-${countryCode}-t${tier}`,
    leagueName,
    season: SEASON,
    logoUrl: teamId ? logoUrlForTeam(teamId) : '',
    source: 'iceland-curated',
    updatedAt: TODAY,
  }
}
...
async function main() {
  ...
  await buildManifest()
  console.log(`\nMerged ${added} curated Icelandic clubs.`)
}
